...
console.log('[Electron] Tools Path:', path.join(WORKSPACE_ROOT, 'tools', 'ai_daemon.py'));
...
function createWindow() {
    let windowState = { width: 1280, height: 800, x: undefined, y: undefined, isMaximized: false };
    try {
...
function startPythonDaemon() {
    const pythonScript = path.join(WORKSPACE_ROOT, 'tools', 'ai_daemon.py');
        ...
        try {
            if (fs.existsSync(APP_STATE_PATH)) {
                ...
                if (appState.lastProjectId) {
                    ...
                    if (currentUrl.endsWith(':3000/') || currentUrl.endsWith('index.html')) {
                        console.log('[Electron] Navigating to last project:', appState.lastProjectId);
