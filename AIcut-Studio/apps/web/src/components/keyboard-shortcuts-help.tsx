    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col p-0">
        <DialogHeader className="flex-shrink-0 p-6 pb-0">
          ...
          <DialogDescription>
            使用这些键盘快捷键显著提升您的视频编辑效率。点击任何按键即可自定义。
          </DialogDescription>
        </DialogHeader>
...
function EditableShortcutKey({
  ...
  return (
    <Button
      variant="outline"
      size="sm"
      className={`font-sans px-2 min-w-6 min-h-6 leading-none mr-1 hover:bg-opacity-80 ${isRecording ? "border-primary bg-primary/10" : "border bg-accent/50"
        }`}
      onClick={handleClick}
      title={
