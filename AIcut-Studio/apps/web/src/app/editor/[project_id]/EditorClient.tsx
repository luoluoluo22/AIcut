    ...
    useEffect(() => {
        ...
        const initProject = async () => {
            ...
            try {
                ...
                // Initialize timeline from snapshot API if available
                try {
                    console.log('[EditorClient] Attempting to load snapshot from API...');
                    const snapshotResponse = await fetch('/api/ai-edit?action=getSnapshot');
                    if (snapshotResponse.ok) {
                        const snapshotData = await snapshotResponse.json();
                        if (snapshotData.success && snapshotData.snapshot?.tracks) {
                            console.log('[EditorClient] Loaded snapshot with', snapshotData.snapshot.tracks.length, 'tracks');
                            // Import tracks from snapshot to timeline store
                            const { useTimelineStore } = await import('@/stores/timeline-store');
                            const timelineStore = useTimelineStore.getState();

                            // Use setTracks which should trigger subscribers
                            timelineStore.setTracks(snapshotData.snapshot.tracks);
                            ...
                            console.log('[EditorClient] ✅ Verification after sync:', {
                                '📊 Tracks count': currentState.tracks.length,
                                '🔧 _Tracks count': currentState._tracks.length,
                                '📝 Track names': currentState.tracks.map(t => t.name),
                                '🎯 Total elements': currentState.tracks.reduce((sum, t) => sum + t.elements.length, 0),
                                '📋 First track': currentState.tracks[0] ? {
                                    id: currentState.tracks[0].id,
                                    name: currentState.tracks[0].name,
                                    type: currentState.tracks[0].type,
                                    elementsCount: currentState.tracks[0].elements.length
                                } : null
                            });
                        }
                    }
                } catch (snapshotError) {
                    console.warn('[EditorClient] Failed to load snapshot:', snapshotError);
                    // Non-critical error, continue with default timeline
                }
